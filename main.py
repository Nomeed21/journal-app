...
def start_book(book_id: int):
    ...
    boosted_xp = ledger_add("book_started", str(book_id), "Personal Growth", BOOK_START_XP)
    new_achievements = award_achievements()
...
def finish_book(book_id: int, data: BookFinish):
    ...
    if not was_finished:
        boosted_xp = ledger_add("book_finished", str(book_id), "Personal Growth", BOOK_FINISH_XP)
        new_achievements = award_achievements()
...
@app.put("/resume/skills/{skill_id}")
...
@app.delete("/resume/skills/{skill_id}")
...
def _build_resume_suggestions() -> dict:
    ...
    try:
        ...
        seen = set()
        for c in completed:
            tree = SKILL_TREES.get(c.get("category"))
            if not tree:
                continue
        ...
        logger.exception("_build_resume_suggestions hard skills failed: %s", e)
    ...
    try:
        ...
        logger.exception("_build_resume_suggestions streak check failed: %s", e)
    ...
    try:
        ...
        logger.exception("_build_resume_suggestions achievements check failed: %s", e)
    ...
    try:
        ...
        logger.exception("_build_resume_suggestions level check failed: %s", e)
    ...
    try:
        ...
        logger.exception("_build_resume_suggestions quest completion check failed: %s", e)
